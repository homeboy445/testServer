...
<script type="text/javascript" id="vwoCode" nonce='testing'>
  window._vwo_code ||
    (function () {
      var account_id =
          new URL(location.href).searchParams.get("accountId") || 644092,
        version = 2.0,
...
app.get("/osc", (req, res) => {
    const { accountId = 1058577 } = req.query;
    const userAgent = req.get('User-Agent');
    console.log(">> UA: ", userAgent);
    const smartCode = `<!-- Start VWO SmartCode --> 
    <script referrerPolicy="no-referrer-when-downgrade" src="https://dev.visualwebsiteoptimizer.com/lib/${accountId}.js" id="vwoCode"></script> 
    <!-- End VWO SmartCode -->`;
    const htmlContent = `<!DOCTYPE html>
    <html>
    <head>
